...
export default class AbstractWindow {
  ...
  getWindowOptions () {
    ...
    if (preloadName) {
      options.webPreferences.preload = path.resolve(app.getAppPath(), 'preloads', `${preloadName}.cjs`);
    }

    options.backgroundColor = this.getBackgroundColor();
    options.titleBarOverlay = this.getTitleBarOverlay();
    options.titleBarStyle = this.getTitleBarStyle();
    ...
    if (process.platform === 'linux') {
      ...
      options.icon = path.resolve(__dirname, '../art/appIcon.png');
    }
  ...
  handleInput (event, input) {
    ...
    if (input.key === 'Escape') {
      if (this.window.isFullScreen() && this.canExitFullscreenByPressingEscape()) {
    ...
    if (process.platform !== 'darwin') {
      ...
      if (input.control && input.key.toLowerCase() === 'n') {
        ...
        import('./editor').then(({default: EditorWindow}) => EditorWindow.newWindow());
      }
      ...
      if (input.control && (input.key === '=' || input.key === '+')) {
        ...
        for (const windows of windowsByClass.values()) {
          for (const window of windows) {
            window.window.webContents.emit('zoom-changed');
          }
        }
      }
